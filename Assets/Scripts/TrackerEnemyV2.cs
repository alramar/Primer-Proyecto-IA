...
public class TrackerEnemyV2 : Enemy
{
    Vector3 steer;
    ...
    void FixedUpdate()
    {
        ...
        // if (timer >= nextWanderTemp)
        // {
        PrepareRayCasts();
        raycastJobHandle = RaycastCommand.ScheduleBatch(commands, results, 3, 1, default);
        jobScheduled = true;
        CalculateFollowPath();
        if (target != null)
        {
        ...
        // else if (path.Any(point => { return Vector3.Distance(transform.position, normalPoint) <= pathMagnetism; }))
        // {
        //     Debug.Log("Followin da path");
        //     FollowPath();
    ...
    void CalculateFollowPath()
    {
        ...
        nextIndex = Mathf.Clamp(nextIndex, 0, path.Length - 1);

        Vector3 a = path[currentPathIndex];
        Vector3 b = path[nextIndex];
        float t;
